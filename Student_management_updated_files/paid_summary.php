...
left join studentfees as  sf on s.studentId = sf.studentId
    WHERE  s.department='".$_POST['department']."' AND s.yearOfStudying='".$_POST['yearOfStudying']."' AND CAST(p.updated_date as date) BETWEEN'".$from_date."'  AND '".$to_date."' GROUP BY s.studentId ORDER BY id DESC") or die(mysqli_error());
...
else
{
  ...
  $result = mysqli_query($connection,"SELECT *,sf.tutionFees,sf.hostelFees,sf.totalFees,sf.tutionFeeDue,sf.travelHostelDue FROM payment_history as p left join student as s on p.student_id = s.studentId
    left join studentfees as  sf on s.studentId = sf.studentId GROUP BY s.studentId
...
<div class="page-wrapper">
  ...
  <div class="page-titles row">
    ...
    $('body').on('click', '.view_challan', function(e){
        ...
        var tot=parseInt(rr.tutionFeeDue, 10) + parseInt(rr.travelHostelDue, 10);
        ...
        var html = '<div class="table-responsive "> '+
               '<table  class="display nowrap table table-hover table-striped table-bordered" >'+
                '<tbody>'+
                  ...
                  '</tr>'+
                   '<tr>'+
                    '<th>Hostel fee Balance</th>'+
                    '<th style="text-align: left;">'+rr.travelHostelDue+'</th>'+
                  '</tr>'+
                  '<tr>'+
                    '<th>Tution Fees Balance</th>'+
                    '<th style="text-align: left;">'+rr.tutionFeeDue+'</th>'+
                  '</tr>'+
                  '<tr>'+
                    '<th>fee Balance</th>'+
