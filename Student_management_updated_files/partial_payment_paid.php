...
left join studentfees as  sf on s.studentId = sf.studentId
    WHERE sf.tutionFeeDue != 0 AND sf.travelHostelDue != 0 AND s.department='".$_POST['department']."' AND s.yearOfStudying='".$_POST['yearOfStudying']."' AND CAST(p.updated_date as date) BETWEEN'".$from_date."'  AND '".$to_date."' GROUP BY s.studentId ORDER BY id DESC") or die(mysqli_error());
}
else
{
  $result = mysqli_query($connection,"SELECT * FROM payment_history as p left join student as s on p.student_id = s.studentId
                ...
                <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                  ...
                  /*  $result = mysqli_query($connection,"SELECT * FROM payment_history as p left join student as s on p.student_id = s.studentId ORDER BY id DESC") or die(mysqli_error());*/
                    ...
                    while($row = mysqli_fetch_assoc($result)) 
                    {
                     ...
                     echo '                        <td>
                      <a href="partial_payment_student_view.php?id='.$row['studentId'].' ">
